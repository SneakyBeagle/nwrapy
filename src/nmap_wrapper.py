...
class Nmap_Wrapper():
    ...
    def __init__(self, *args, **kwargs):
        ...
        stdout=stdout.split('\n')[0]
        if stderr:
            print(clr.RED+'[!] nmap was not found:', stderr, clr.RESET, file=sys.stderr)
            sys.exit(1)
        else:
    ...
    def send_icmp_echo(self, target):
        self.execute_nmap(target=target,
    ...
    def execute_nmap(self, target, cmd, xml_file, options):
        xml_file = os.path.join(self.xml_dir, xml_file.format(target=target).replace('/','_'))
